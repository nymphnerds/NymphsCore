...
### Workflow order for texture, parts, and PBR

The likely product workflow is not strictly linear.

Current recommended reading:

- `Z-Image` creates or improves guide images
- `Hunyuan 2mv` or `TRELLIS.2` creates the base mesh
- then the user chooses one branch:
  - texture now
  - or split/decompose parts first
- Blender-side PBR and node assembly should happen after the mesh workflow is
  clearer, not before

Important rule:

- parts decomposition should stay an optional post-generation branch
- do not force it ahead of every texture workflow
- keep `TRELLIS.2` same-pass shape + texture as a valid normal path

Open practical concern:

- if a mesh is split into parts before texturing, each part may need a better
  part-specific guide image than the original whole-object guide
- that means parts-aware texturing should be treated as its own later workflow
  problem, not assumed to be solved by the current whole-object texture flow

Texture preservation note:

- texture and material preservation across decomposed parts is in scope for the
  addon, but should be treated as a later post-decomposition layer rather than
  something the public `P3-SAM` / `X-Part` release already solves
- the current public parts pipeline mainly gives:
  - analysis
  - part geometry generation
  - debug / segmentation coloring
- it does not currently provide production-ready preservation of the original
  textured mesh materials, UVs, or full PBR set
- likely addon-side future approaches include:
  - transferring material assignment from the source mesh onto generated parts
  - rebaking source textures onto decomposed geometry
  - part-aware retexture workflows after decomposition
  - Blender-side material/node reconstruction after import
- sequencing matters:
  - prove decomposition quality first
  - then solve texture/material carryover
  - then solve higher-end PBR finishing
- use the current textured-mesh parts tests as geometry-quality evaluation
  first, not as a proof that texture preservation already works

### Nymphs Parts and P3-SAM product pass

Status:

- `P3-SAM` now has a proven real Blender-launched path from selected mesh to
  output files
- local test builds through `1.1.71` fixed the first round of launch blockers:
  - Windows Blender detecting WSL-only repo/python paths
  - prepared Windows temp meshes being converted into WSL `/mnt/c/...` paths
  - background worker snapshot state being used correctly
  - per-run log capture into each parts output folder
- first successful selected-mesh run produced:
  - `p3sam_segmented.glb`
  - `p3sam_segmented.ply`
  - AABB outputs
...
- evaluate whether the `p3sam_segmented.glb` result is visually useful in
  Blender
- inspect why the wrapper summary reported `part_count=2` while upstream logs
  also described more connected mesh regions during post-processing
- decide whether addon summary should expose final labels, connected regions, or
  both
- improve progress/status text so the user can see meaningful milestones during
  a run instead of relying on GPU activity
- improve result import behavior:
  - place parts output into the requested collection
  - preserve source transform cleanly
  - make source hiding predictable
  - expose output folder and log as clear buttons
- rename or restructure confusing UI:
  - `Experimental Backend` should likely become `P3-SAM Setup`
  - make it explicit that `P3-SAM` is a one-shot job, not a Runtimes server
  - hide or clearly fence off `X-Part` and decomposition until they are actually
    usable
- add better plain-English mouseover descriptions for the parts knobs:
  - Points
  - Prompts
  - Prompt Batch
- decide whether Parts belongs only in its own panel or also gets a config/status
  card in Runtimes without pretending it has Start/Stop server behavior

Medium-term parts direction:

- keep `P3-SAM` as the practical first segmentation lane
- keep `X-Part` as research-only until its public release can be made reliable
- explore part-aware texturing only after segmentation/import UX is stable
- treat real decomposition into separately editable assets as a later workflow,
  not as something already solved by the current segmentation output
...
### `Hunyuan3D-Part` as a research lane

`Hunyuan3D-Part` is worth tracking as a future mesh-editing and decomposition
tool, but not as a baseline install requirement.

Why it is interesting:

- it may offer a path toward part-aware mesh editing after generation
- it could become useful for decomposition, cleanup, or downstream rigging prep
- it aligns better with a later mesh-operations workflow than with the current
  first-run installer baseline
- the public release is currently a lighter public lane rather than the fullest
  internal/studio version, so expectations should stay experimental

Current public reading:

- `P3-SAM` looks useful as a broad mesh segmentation tool
- `X-Part` is interesting for decomposition, but the current public release is
  described as a light version
- treat the current public lane as a research/prototype target, not a core
  shipped dependency yet

How it should be treated for now:

- keep it on the roadmap as a research item
- test it against current addon-imported meshes when time allows
- do not fold it into the default installer or addon startup surface until it
  proves a clear workflow win
- if it earns a UI surface, give it its own `Nymphs Parts` panel rather than
  burying it inside Shape or Texture
- evaluate it after the texture panel cleanup, but before a full Blender-side
  PBR panel, because parts may change the mesh/material workflow in ways that
