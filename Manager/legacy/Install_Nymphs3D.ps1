...
$RepoDir = Join-Path $ExtractRoot "Nymphs3D-$RepoBranch"
$InstallerPath = Join-Path $RepoDir "scripts\install_one_click_windows.ps1"

if (-not $NonInteractive.IsPresent) {
    Write-Host ""
    Write-Host "Welcome to the Nymphs3D installer."
    Write-Host ""
    Write-Host "This installer sets up the local backend needed for Nymphs3D on this PC."
    Write-Host ""
...
Write-Host "Preparing Nymphs3D installer..."
