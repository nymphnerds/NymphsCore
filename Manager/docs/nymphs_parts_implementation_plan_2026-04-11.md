...
- `X-Part` has now completed a minimal local Stage-2 proof from saved
  Stage-1 `aabb`
...
- stage 1 is runnable
- stage 2 is runnable in a conservative proof lane
- stage-1 output quality and scaling remain open questions
- the UI can now be refactored around the real two-stage pipeline, but should
  expose conservative defaults first
...
6. refactor the addon panel from backend-toggle to pipeline
7. after that:
   - improve progress UX
...
Do not prioritize these before the conservative Stage-2 lane is wired cleanly:

- more cosmetic `P3-SAM` panel polish
- more `flash-attn` work for the parts venv
...
- addon wiring for the proven one-box Stage-2 contract
- careful scaling beyond the minimal proof lane
...
- `X-Part` is now locally proven as the intended second stage in a conservative
  one-box lane
...
- the next engineering milestone is not "more P3-SAM", it is:
  ...
  - wire stage 2 cleanly in the addon
  - scale beyond the one-box proof lane carefully
